...
This repository contains a SwiftUI photo-review app targeting iOS 26+ and a GitHub Actions builder for an unsigned iOS IPA.
...
The workflow selects Xcode 26, archives for a generic iOS device, and packages the app as `Payload/<App>.app` inside the IPA. It deliberately disables Apple code signing, so the artifact must be re-signed by AltStore/SideStore or another provisioning workflow before installing on a device. It is not an App Store distribution build.
